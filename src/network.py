...
class MulticastServer(threading.Thread):
    ...
    def run(self):
        ...
        sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP,
                        socket.inet_aton(SSDP_MCAST_ADDR) + struct.pack('I', socket.INADDR_ANY))
        ...
        while not self.stop_listening.isSet():
            ...
            if sock in readset:
                ...
                assert len(data) < self.buffer_size, len(source_address)
...
def send_unicast_message(address, message):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
